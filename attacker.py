...
class PGDAttacker():
    def __init__(self, num_iter, epsilon, step_size,num_classes = 100, kernel_size=15, prob_start_from_clean=0.0, translation=False, device='cuda:0'):
    ...
    def _create_random_target(self, label):
        label_offset = torch.randint_like(label, low=0, high=200)
        return (label + label_offset) % self.num_classes

    def attack(self, image_clean, label, model, testing = False, original=False):#set the original from False to True don't change the label and just find the most misleading one
        if original:
            target_label = label
        else:
        ...
        for i in range(self.num_iter):
            ...
            if testing:
                logits,_ = model(adv,target_label)
            else:
                logits = model(adv) #original code
            # logits,_ = model(adv,target_label) # add target label to the model, which can help with adversial testing when attacker is outside the model
