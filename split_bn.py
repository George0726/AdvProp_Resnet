...
import os
from train_nasty import MixBatchNorm2d,test
...
normal_dir = "checkpoint_compile/normal_bn"
aux_dir = "checkpoint_compile/aux_bn"

#model pre-load
model = net_rectified.__dict__['resnet50'](num_classes=100 ,norm_layer=MixBatchNorm2d)
...
checkpoint = torch.load("distill/cifar100/nasty_resnet50/model_best.pth.tar")
...
model_bn = net_rectified.__dict__['resnet50'](num_classes=100 ,norm_layer=None)
model_bn.set_attacker(NoOpAttacker)
model_bn.set_mixbn(False)
# model1 = torch.nn.DataParallel(model1).cuda()

model_aux = net_rectified.__dict__['resnet50'](num_classes=100 ,norm_layer=None)
