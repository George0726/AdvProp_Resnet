...
# Models
default_model_names = sorted(name for name in net.__dict__ if name.islower() and not name.startswith('__') and callable(
    net.__dict__[name]) and not name.startswith("to_") and not name.startswith("partial"))
...
parser.add_argument('--mixbn', action='store_true', help='use mixbn', default=True)
...
parser.add_argument('--attack-iter', help='Adversarial attack iteration', type=int, default=4)
parser.add_argument('--attack-epsilon', help='Adversarial attack maximal perturbation', type=float, default=5.0)
