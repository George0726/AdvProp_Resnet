...
# Checkpoints
parser.add_argument('-c', '--checkpoint', default='checkpoint/distill/', type=str, metavar='PATH',
                    help='path to save checkpoint (default: checkpoint)')

parser.add_argument("--Temperature", default= 20, type= int)
parser.add_argument("--alpha_distill", default= 0.9, type = float)
...
# Architecture
parser.add_argument('--arch_S', '-a_S', metavar='ARCH', default='resnet18',
                    choices=model_names,
                    help='model architecture: ' +
                         ' | '.join(model_names) +
                         ' (default: resnet18)')

parser.add_argument('--arch_T', '-a_T', metavar='ARCH', default='resnet50',
                    choices=model_names,
                    help='model architecture: ' +
                         ' | '.join(model_names) +
                         ' (default: resnet18)')
...
parser.add_argument('--evaluate_adv', action='store_true',
                    help='evaluate model on validation set')
# Device options
...
def main():
    ...
    if args.mixbn:
        ...
        norm_layer = None
    # model initilization
    teacher_model = net_rectified.__dict__[args.arch_T](num_classes=num_classes, norm_layer=norm_layer)
    teacher_model.set_attacker(attacker)
    teacher_model.set_mixbn(args.mixbn)
    if args.ngpu >1:
    ...
    #Qirui modified, model loading
    if args.teacher_model:
        checkpoint = torch.load(args.teacher_model)
    else:
        print("feed in the adv loss of function")
        exit()
    checkpoint = torch.load(args.teacher_model)
    ...
    student_model = net_rectified.__dict__[args.arch_S](num_classes=num_classes, norm_layer=norm_layer)
    student_model.set_attacker(attacker)
    student_model.set_mixbn(args.mixbn)
    if args.ngpu >1:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_func = partial(train, train_loader=train_loader,T_model = teacher_model, S_model=student_model, criterion=criterion,
        ...
        if args.mixbn:
            teacher_model.apply(to_mix_status)
            student_model.apply(to_mix_status)
        ...
        if args.mixbn:
            ...
            student_model.apply(to_clean_status)
        test_loss, test_acc = test(val_loader, student_model, criterion, epoch, use_cuda)
...
def train(train_loader, T_model,S_model, criterion, optimizer, epoch, use_cuda, warmup_scheduler, mixbn=False,
          writer=None, attacker=NoOpAttacker(),num_classes=10):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        # Qirui modified: loss function from nasty training

        outputs_S, aux_target, aux_imgs = S_model(inputs, targets, True)
        # print(aux_imgs.shape)
        targets = aux_target.transpose(1, 0).contiguous().view(-1)
        # tch_loss = nn.CrossEntropyLoss()()
        with torch.no_grad():
            outputs_T, _ = T_model(aux_imgs, targets)

        # Modified by Qirui
        # the adversarial model only get normal value during the training, concate the same data for calculating the loss of adverserial data 
        # outputs_T = torch.cat([outputs_T, outputs_T], dim=0).view(2, len(inputs), -1).transpose(1, 0)
        # transfer [128, 100] to [128, 2, 100]
        # print(outputs_S.shape,outputs_T.shape,targets.shape)
        # torch.Size([128, 2, 100]) torch.Size([128, 100]) torch.Size([128, 2])

        if args.mixbn:
            outputs_T = outputs_T.transpose(1, 0).contiguous().view(-1, num_classes)
            # targets = targets.transpose(1, 0).contiguous().view(-1)
            outputs_S = outputs_S.transpose(1, 0).contiguous().view(-1, num_classes)

            outputs_T = outputs_T.detach()

        # print(outputs_S.shape,outputs_T.shape,targets.shape)
        # torch.Size([256, 100]) torch.Size([128, 100]) torch.Size([256])
        ###############  distillation loss ########################
        # modified by Qirui adapted from redistillation

        T = args.Temperature
        alpha = args.alpha_distill
        loss = nn.KLDivLoss(reduction='batchmean')(F.log_softmax(outputs_S / T, dim=1),
                                    F.softmax(outputs_T / T, dim=1)) * (alpha * T * T)  + nn.CrossEntropyLoss()(outputs_S,targets) * (1. - alpha) # wish to max this item

        # measure accuracy and record loss
        prec1, prec5 = accuracy(outputs_S.data, targets.data, topk=(1, 5))
        losses.update(loss.item(), outputs_S.size(0))
        top1.update(prec1.item(), outputs_S.size(0))
        top5.update(prec5.item(), outputs_S.size(0))

        if mixbn:
            with torch.no_grad():
                batch_size = outputs_S.size(0)
                loss_main = criterion(outputs_S[:batch_size // 2], targets[:batch_size // 2]).mean()
                loss_aux = criterion(outputs_S[batch_size // 2:], targets[batch_size // 2:]).mean()
                prec1_main = accuracy(outputs_S.data[:batch_size // 2],
                                      targets.data[:batch_size // 2], topk=(1,))[0]
                prec1_aux = accuracy(outputs_S.data[batch_size // 2:],
                                     targets.data[batch_size // 2:], topk=(1,))[0]
