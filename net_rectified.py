...
class AdvResNet(ResNet):
    ...
    so you'd better set the attacker and mixbn before using DataParallel.ß
    '''
    ...
    def forward(self, x ,labels,is_distill =False):
        ...
        if training:
            ...
            if isinstance(self.attacker, NoOpAttacker):
                ...
                aux_images, aux_label = self.attacker.attack(x, labels, self._forward_impl)
            ...
            if self.mixbn:
                if is_distill:
                    ...
                    return self._forward_impl(images).view(2, input_len, -1).transpose(1, 0), targets.view(2,input_len).transpose(1, 0),images
                else:
                    ...
                    self.apply(to_mix_status)
                    return self._forward_impl(images).view(2, input_len, -1).transpose(1, 0), targets.view(2,
                                                                                                        input_len).transpose(
                        1, 0)
            else:
...
def _resnet( block, layers, pretrained, progress, num_classes, **kwargs):
    model = AdvResNet(block, layers, num_classes, **kwargs)
    if pretrained:
...
def resnet18(num_classes = 200, pretrained=False, progress=True, **kwargs):
    ...
    """
    return _resnet( BasicBlock, [2, 2, 2, 2], pretrained, progress, num_classes,
                   **kwargs)
